...
char search_word[20][COUNT]={"The","around","graphics","form","by","be","any","which","various","mount"};
long counts[COUNT];
int line_c=0;
...
long determine_count(const char*file_name, const *file_name,const char*key,int ignore_case)
 {
   ...
   while(feof(fp)==0)
    {
      read_word(temp,fp);
      if(is_equal(temp,keymignore_case)!=0)
        {
          word_count++;
        }
    }
